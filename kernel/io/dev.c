...
void init_devices(struct fakix_sdt *rsdt)
{
	...
	struct mcfg_entry *mcfg_ent = (void *)((uint8_t *)header + sizeof *header + 8);

	puts("Enumerating devices...");
	size_t nentries = (header->length - (sizeof *header + 8)) / sizeof(struct mcfg_entry);
	printf("Iterating through %lx configuration address spaces, starting at %#lx...\n", nentries, mcfg_ent);
	while (nentries-- > 0) {
...
static void check_function(struct mcfg_entry *config_space, int bus, int device, int function);
...
static int get_vendor_id(struct mcfg_entry *config_space, int bus, int device, int function);
static int get_class(struct mcfg_entry *config_space, int bus, int device, int function);
static int get_subclass(struct mcfg_entry *config_space, int bus, int device, int function);
static int get_progif(struct mcfg_entry *config_space, int bus, int device, int function);
static int get_header_type(struct mcfg_entry *config_space, int bus, int device, int function);
static int get_secondary_bus(struct mcfg_entry *config_space, int bus, int device, int function);
...
{
	uintptr_t base = config_space->base;
	struct mcfg_config_header *cfg = conv_phys_to_kern(
		(base + (((bus - config_space->startbus) << 20) |
		(device << 15) | 
		(function << 12)))
	);
