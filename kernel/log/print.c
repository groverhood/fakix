...
static void kernel_vsnprintf(char *dest, size_t buflen, const char *format, va_list argv);

void kernel_snprintf(char *dest, size_t buflen, const char *format, ...)
{
    va_list argv;
    va_start(argv, format);
    kernel_vsnprintf(dest, buflen, format, argv);
    va_end(argv);
}
...
static char *strchr(const char *str, int chr)
{
    int c;
    while ((c = *str) != chr) {
        str++;
