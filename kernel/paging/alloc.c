...
void init_page_alloc(struct fakix_memmap *mmap)
{
	...
	for (i = 0; i < MAX_REGIONS; ++i) {
		...
		struct page_pool *pg_pool = conv_phys_to_kern(coalesced_regions[i].base_address);
...
static bool overlapping_regions(struct fakix_memmap *map1, struct fakix_memmap *map2)
{
	...
	for (i = 0; i < lim; ++i) {
		...
		if (mmap[i].type == 1) {
			...
			for (j = i + 1; j < lim; ++j) {
				...
				if (mmap[j].type == 1 && overlapping_regions(newmmap, mmap + j)) {
